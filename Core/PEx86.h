	...
	if (std::filesystem::exists (FilePath))
	{
	  ...
	  if (ModuleBase)
	  {
		...
		if (file.read ((char *)ModuleBase, FileSize))
		{
		  ...
		  {
			DebugData = GET_HEADER_DICTIONARY<PIMAGE_NT_HEADERS32> (
				ModuleBase, IMAGE_DIRECTORY_ENTRY_DEBUG);
		  }
		  else if ((NT->FileHeader.Machine == IMAGE_FILE_MACHINE_AMD64) || (NT->FileHeader.Machine == IMAGE_NT_OPTIONAL_HDR64_MAGIC))
		  {
			DebugData = GET_HEADER_DICTIONARY<PIMAGE_NT_HEADERS64> (
				ModuleBase, IMAGE_DIRECTORY_ENTRY_DEBUG);
		  }
		  else
		  {
		  ...
		  if (DebugData)
		  {
			...
			if (Offset)
			{
			  ...
			  for (int i = 0; i < count; ++i)
			  {
				...
				{
				  auto debug =
					  (PVOID)((PBYTE)ModuleBase + DebugEntry->PointerToRawData);
				  ...
				  case PDB20: {
					...
					char time_age[10];
					snprintf (time_age, sizeof (time_age), "%08X%x", timestamp,
							  age);
