...
namespace PDBParser
{
  ...
  WINBOOL EnumSymCallBack (PSYMBOL_INFO pSymInfo,
						   [[maybe_unused]] ULONG SymbolSize,
   ...
   */
	...
	if (CurrentHandle != INVALID_HANDLE_VALUE)
	{
	  ...
	  if (SymbolsPathInit)
	  {
		...
		SIZE_T SymBase = SymLoadModule (CurrentHandle, nullptr, FilePath.c_str (),
										nullptr, 0, 0);
		if (SymBase != 0)
		{
		  return SymEnumSymbols (CurrentHandle, SymBase, nullptr, EnumSymCallBack,
								 nullptr);
		}
	  }
	}
