...
{
  HRESULT hr = URLDownloadToFileA (nullptr, PDBUrl.c_str (), FilePath.c_str (),
								   BINDF_GETNEWESTVERSION, nullptr);
  return hr == S_OK;
}
...
int main (int argc, char **argv)
{
  ...
  if (fs::exists (PDBFile))
  {
	...
	PDBParser::UserSymbols.insert (
		"PsSetCreate");// Any Symbol contains "PsSetCreate".
	PDBParser::LoadAndCheckSym (
		FilePath,
		PDBFile);// Load the PDB file and Parse it using Imagehlp APIs.
  }
